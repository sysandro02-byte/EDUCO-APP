...
const AdminSubscriptionHub: React.FC<AdminSubscriptionHubProps> = ({ onSelectSchool }) => {
  ...
  const handleGenerateCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!schoolName.trim() || !promoterName.trim()) {
      alert('Veuillez renseigner le nom de l\'école et du promoteur.');
      return;
    }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSchoolIdentifier(e.target.value.toUpperCase())}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-mono font-bold focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#1F4A59]"
                />
              </div>
