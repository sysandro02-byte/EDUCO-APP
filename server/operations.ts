...
export async function getCashierSettingsForUser(client: any, user: any) {
  if (!client || !user?.schoolId) return normalizeCashierSettings(null);
  const { data: school, error } = await client.from('schools').select('settings').eq('id', user.schoolId).single();
  if (error) throw error;
  return normalizeCashierSettings(school?.settings?.operations?.cashierSettings);
}
...
export function mutateOperations(current: any, key: string, body: any) {
  if (!collections.has(key)) return { ...current, [key]: key === 'cashierSettings' ? normalizeCashierSettings(body.value) : body.value };
...
export function registerOperations(app: Express, requireAuth: any, getUser: any, getClient: any) {
  ...
  app.post('/api/records/:table/delete', requireAuth, async (req, res) => {
    try {
      ...
      const user = await getUser(req); if (!user?.schoolId || !allowed[table]?.includes(canonicalizeRole(user.role))) return res.status(403).json({ error: 'Suppression non autorisée.' });
      const client = getClient(req); if (!client) throw new Error('Supabase non configuré.');
      const { data, error } = await client.from(table).delete().eq('school_id', user.schoolId).eq('id', req.body.id).select('id');
      if (error) throw error; if (!data?.length) return res.status(404).json({ error: 'Enregistrement introuvable.' }); res.json({ success: true });
    } catch (error: any) { res.status(500).json({ error: error.message }); }
  });
  async function context(req: any) {
    const user = await getUser(req); if (!user?.schoolId) throw new Error('Aucun établissement associé au compte.');
    const client = getClient(req); if (!client) throw new Error('Supabase non configuré.');
    const { data: school, error } = await client.from('schools').select('*').eq('id', user.schoolId).single(); if (error) throw error;
    return { user, client, school };
  }
  app.get('/api/operations', requireAuth, async (req, res) => {
    try {
      const { user, client, school } = await context(req);
      const results = await Promise.all([client.from('classes').select('*').eq('school_id', user.schoolId), client.from('students').select('*').eq('school_id', user.schoolId), client.from('subjects').select('*').eq('school_id', user.schoolId)]);
      for (const result of results) if (result.error) throw result.error;
      const [classRows, students, subjectRows] = results.map(result => result.data || []); const allClassIds = classRows.map((row: any) => row.id);
      const legacy = allClassIds.length ? await Promise.all([client.from('attendance').select('*').in('class_id', allClassIds), client.from('timetable').select('*').in('class_id', allClassIds)]) : [{ data: [] }, { data: [] }];
      for (const result of legacy) if (result.error) throw result.error;
      const saved = school.settings?.operations || {}; const data: any = { ...saved };
      for (const key of collections) data[key] = saved[key] || [];
      data.cashierSettings = normalizeCashierSettings(saved.cashierSettings);
      ...
      if (personalRole(userRole)) {
        const linked = selectPersonalStudents(user, students); const classIds = linked.map((s: any) => s.class_id); const studentIds = linked.flatMap((s: any) => [s.id, s.user_id]);
  ...
  app.post('/api/operations/:key', requireAuth, async (req, res) => {
    try {
      const key = String(req.params.key); const { user, client } = await context(req); const userRole = canonicalizeRole(user.role);
      if (!operationRoles[key]?.includes(userRole)) return res.status(403).json({ error: 'Opération non autorisée pour ce compte.' });
      const body = req.body; if (body.action !== 'delete' && key !== 'attendance' && (!body.value || typeof body.value !== 'object' || Array.isArray(body.value))) return res.status(400).json({ error: 'Données invalides.' });
      ...
      if (classId) { const { data: schoolClass, error } = await client.from('classes').select('*').eq('id', classId).eq('school_id', user.schoolId).maybeSingle(); if (error) throw error; if (!schoolClass || (userRole === 'Enseignant' && !same(schoolClass.teacher_id, user.id))) return res.status(403).json({ error: 'Classe non autorisée.' }); }
      if (key === 'attendance' || key === 'reportCardComments') { const ids = key === 'attendance' ? body.records.map((row: any) => row.studentId) : [body.value.studentId]; const { data: pupils, error } = await client.from('users').select('id,role').eq('school_id', user.schoolId).in('id', ids); if (error) throw error; if (ids.some((id: any) => !pupils.some((p: any) => same(p.id, id) && /élève|eleve/i.test(p.role)))) return res.status(403).json({ error: 'Élève non autorisé.' }); }
      for (let attempt = 0; attempt < 4; attempt++) {
        const { data: school, error } = await client.from('schools').select('settings').eq('id', user.schoolId).single(); if (error) throw error;
        const old = school.settings; const current = { ...(old?.operations || {}) };
        if (current[key] === undefined && (key === 'subjects' || key === 'timetable')) {
          if (key === 'subjects') { const legacy = await client.from('subjects').select('*').eq('school_id', user.schoolId); if (legacy.error) throw legacy.error; current.subjects = legacy.data || []; }
          else { const schoolClasses = await client.from('classes').select('id').eq('school_id', user.schoolId); if (schoolClasses.error) throw schoolClasses.error; const ids = (schoolClasses.data || []).map((c: any) => c.id); const legacy = ids.length ? await client.from('timetable').select('*').in('class_id', ids) : { data: [] }; if (legacy.error) throw legacy.error; current.timetable = (legacy.data || []).map((row: any) => ({ id: String(row.id), classId: row.class_id, subjectId: row.subject_id, teacherId: row.teacher_id, day: row.day_of_week, startTime: row.start_time, endTime: row.end_time })); }
        }
        const next = { ...(old || {}), operations: mutateOperations(current, key, body) }; let update = client.from('schools').update({ settings: next }).eq('id', user.schoolId); update = old == null ? update.is('settings', null) : update.eq('settings', JSON.stringify(old));
        const result = await update.select('id'); if (result.error) throw result.error; if (result.data?.length) return res.json({ success: true, value: next.operations[key] });
      }
