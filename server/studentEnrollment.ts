...
const isDuplicateError = (error: any) => error?.code === '23505' || /duplicate|unique|already exists|already registered/i.test(String(error?.message || error || ''));
...
async function cashierCanRegister(client: any, schoolId: number) {
  const { data: school, error } = await client.from('schools').select('settings').eq('id', schoolId).maybeSingle();
  if (error) throw error;
  return school?.settings?.operations?.cashierSettings?.permissions?.allowRegistration === true;
}
...
export function registerStudentEnrollment(app: Express, requireAuth: any, getUser: any, getClient: any) {
  ...
  app.post('/api/enrollments/students', requireAuth, async (req: any, res) => {
    ...
    try {
      ...
      const requestedRole = canonicalizeRole(req.body?.role || STUDENT_ROLE);
      if (requestedRole !== STUDENT_ROLE) {
      ...
      if (actorRole === 'Caissière' && !(await cashierCanRegister(client, schoolId))) {
        return res.status(403).json({ error: 'La caissière n’est pas autorisée à effectuer des inscriptions.' });
      }
      ...
      const { data: authResult, error: authError } = await client.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { name, role: STUDENT_ROLE, schoolId },
      });
      ...
      if (userInsert.error || !userInsert.data) {
        if (isDuplicateError(userInsert.error)) {
          const message = /phone/i.test(String(userInsert.error?.message || ''))
      ...
      if (studentInsert.error || !studentInsert.data) {
        if (isDuplicateError(studentInsert.error)) {
          throw Object.assign(new Error('Ce matricule élève ou ce compte possède déjà un dossier scolaire.'), { status: 409 });
        }
